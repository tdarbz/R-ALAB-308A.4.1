...
const API_KEY =
  "live_kQZiEzPtjWxG0S1GEpynAuOEZnUEw8rnwMnbLpFxmgLP3SBMPocD7XTzWKgc1Sdc";
...
async function initialLoad() {
  try {
    ...
    // Call the event handler for breed selection
    breedSelect.addEventListener("change", handleBreedSelect);

    // Call handleBreedSelect to initially populate the carousel
    handleBreedSelect();
  } catch (error) {
...
async function handleBreedSelect() {
  ...
  try {
    ...
    // Update the infoDump section with breed information
    infoDump.innerHTML = `<h2>${images[0].breeds[0].name}</h2>
...
// Add a default header with the API key for all requests
const headers = {
  "x-api-key": API_KEY,
};
...
// Axios onDownloadProgress config option
axios.interceptors.request.use(
  function (config) {
